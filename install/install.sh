...
[Service]
Type=simple
ExecStart=/usr/local/bin/taildog up --foreground
Restart=on-failure
RestartSec=5
...
UNIT
      ...
      sudo systemctl enable --now taildog
      echo "systemd service enabled and started."
    else
      echo "systemd not available; start manually with: taildog up"
    fi
    ;;
...
<dict>
  <key>Label</key>
  <string>dog.tail.taildog</string>
  <key>ProgramArguments</key>
  <array>
    <string>/usr/local/bin/taildog</string>
    <string>up</string>
  ...
  <string>/tmp/taildog.err</string>
  <key>StandardOutPath</key>
  <string>/tmp/taildog.out</string>
</dict>
</plist>
PLIST
...
echo ""
echo "taildog installed. Run: taildog up"
